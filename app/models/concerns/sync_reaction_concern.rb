module SyncReactionConcern
  ...
  included do
    ...
    sync_has_data :reactionSummary, type: { like?: :int, dislike?: :int }, preload: :reaction_summary_loader do |preloaded|
      preloaded[id] || {}
    end

    sync_has_one :myReaction, type: -> { Reaction }, preload: :my_reaction_loader do |preloaded|
      preloaded[id]
    end
  end
end
